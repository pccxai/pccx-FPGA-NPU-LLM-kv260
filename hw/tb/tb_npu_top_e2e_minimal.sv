...
//     mmio_npu_stat done pushed through AXIL_STAT_OUT by a TB-only overlay
//
// Claim guard
// -----------
//   This is intentionally TB-only. It uses hierarchical force overlays for two
//   known integration gaps in the current top-level RTL: scheduler uops do not
//   carry valid bits into mem_dispatcher, and mmio_npu_stat is not yet wired to
//   ctrl_npu_frontend.IN_enc_stat. The PASS verdict covers the intended public
//   integration contract and must not be cited as proof that those RTL wires
...
module tb_npu_top_e2e_minimal;
  ...
  // ===| TB-only overlays |=====================================================
  task automatic apply_status_overlay;
    begin
      force dut.u_npu_controller_top.u_npu_frontend.IN_enc_stat = {32'd0, dut.mmio_npu_stat};
      force dut.u_npu_controller_top.u_npu_frontend.IN_enc_valid = dut.mmio_npu_stat[1];
    end
  endtask
