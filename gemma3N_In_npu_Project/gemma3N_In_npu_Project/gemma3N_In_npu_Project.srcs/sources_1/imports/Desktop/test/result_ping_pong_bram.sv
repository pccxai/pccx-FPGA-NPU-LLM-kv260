...
module result_ping_pong_bram #(
    ...
    always_comb begin
        for (int i = 0; i < ARRAY_SIZE; i++) begin
            npu_flat_data[i*32 +: 32] = npu_data_in[i];
        end
    end

    logic [1023:0] rdata_0, rdata_1;
    logic          we_0, we_1;
    ...
    // BRAM 0 (1024-bit Wide)
    (* ram_style = "block" *) logic [1023:0] mem_0 [0:(1<<ADDR_WIDTH)-1];
    always_ff @(posedge clk) begin
        if (we_0) mem_0[npu_addr] <= npu_flat_data;
        rdata_0 <= mem_0[dma_addr];
    end

    // BRAM 1 (1024-bit Wide)
    (* ram_style = "block" *) logic [1023:0] mem_1 [0:(1<<ADDR_WIDTH)-1];
    always_ff @(posedge clk) begin
