...
module gemma_layer_top (
    ...
    ping_pong_bram #(
        ...
        .dma_write_data(i_dma_wdata_token), // {504'd0, ...} Erase and connect directly!        .npu_addr_a({3'd0, feed_counter}), // 6bit -> 9bit expansion
    ...
    assign mac_valid_out = shift_reg_valid[63]; // PE completed at bottom right after about 64 clocks

    // // =========================================================================
    // 5. [Stage 3] Output Buffering & Activation MUX
    // =========================================================================
    // [Pure method] Save 32-bit calculation results of 32 channels at once in BRAM (prevent DSP pruning)
    logic [31:0] mac_result_row [0:31];
    always_comb begin
        for (int j = 0; j < 32; j++) begin
            // Extract last row (Row 31) data from flat bus
            mac_result_row[j] = mac_out_acc_flat[(31*32 + j)*32 +: 32];
        end
    end

    // Result address counter (blocks Vivado Pruning by making it dynamic rather than a fixed address!)
